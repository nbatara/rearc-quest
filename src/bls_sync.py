...
def crawl_index(session: BLSRequestSession, base_url: str) -> List[str]:
    ...
    # TODO: Implement crawler that fetches directory listing and returns keys
    return []
...
def perform_sync(config: BLSSyncConfig) -> S3SyncResult:
    ...
    LOGGER.info("Starting S3 sync", extra={"bucket": config.bucket, "prefix": config.prefix})
    return sync_s3_objects(destination=config.destination(), object_keys=objects, session=session)
